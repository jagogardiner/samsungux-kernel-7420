...
echo
echo "${bldcya}***** Make flashable zip *****${txtrst}"

cp -R ./otw ${KERNELDIR}/output/$TARGET/
cp ./$TARGET/boot.img ${KERNELDIR}/output/$TARGET/otw
...
tar -H ustar -c ${KERNELDIR}/output/$TARGET/otw/boot.img > SM-$TARGET-$AUDIO-kernel-${GETVER}-`date +[%d-%m-%y]`.tar
...
echo
echo "Done making flashable zip!"
